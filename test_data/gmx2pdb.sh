...
gmx select -s "${system}_solvated.gro" -on "${system}_substrate.ndx" \
  -select 'resname SURF'
gmx select -s "${system}_solvated.gro" -on "${system}_surfactant.ndx" \
  -select 'resname SDS CTAB'
gmx select -s "${system}_solvated.gro" -on "${system}_solvent.ndx" \
  -select 'resname SOL'
gmx select -s "${system}_solvated.gro" -on "${system}_ions.ndx" \
...
for component in ${components[@]}; do
    ...
    gmx editconf -f "${system}_solvated.gro" -n "${system}_${component}.ndx" \
